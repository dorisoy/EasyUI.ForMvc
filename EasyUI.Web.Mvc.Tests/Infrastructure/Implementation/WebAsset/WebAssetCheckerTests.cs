namespace EasyUI.Web.Mvc.Infrastructure.Implementation.Tests
{
    ...
    public class WebAssetCheckerTests
    {
        ...
        [Fact]
        public void IsNative_returns_true_for_script_starting_with_easyui()
        {
            Assert.True(checker.IsNative(new WebAsset("easyui.foo")));
        }

        [Fact]
        public void IsNative_checks_the_file_name_only()
        {
            Assert.True(checker.IsNative(new WebAsset("~/scripts/easyui.foo")));
        }

        [Fact]
        public void IsAbsolute_returns_false_for_local_urls()
        {
            Assert.False(checker.IsAbsolute(new WebAsset("~/scripts/easyui.foo")));
        }
